...
cmake -S "$ROOT_DIR" -B "$BUILD_DIR" \
  ...
  -DFS2GDL90_BUILD_TESTS=ON \
  -DFS2GDL90_ENABLE_COVERAGE=ON \
  -DCMAKE_BUILD_TYPE=Debug

cmake --build "$BUILD_DIR" --target fs2gdl90_tests --parallel
...
SUMMARY_JSON="$("$LLVM_COV" export "$BUILD_DIR/fs2gdl90_tests" \
  "${ARCH_FLAG[@]}" \
...
ROOT = os.environ["FS2GDL90_ROOT_DIR"]
...
"$LLVM_COV" report "$BUILD_DIR/fs2gdl90_tests" \
  "${ARCH_FLAG[@]}" \
  -instr-profile="$BUILD_DIR/coverage.profdata" \
  --ignore-filename-regex="$IGNORE_REGEX"
