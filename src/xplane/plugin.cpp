...
using fs2gdl90::Settings;
using fs2gdl90::SettingsUiState;
...
void LogMessage(const std::string &message) {
  XPLMDebugString(("[FS2GDL90] " + message + "\n").c_str());
}
...
std::string ReadTrafficCallsign(size_t slot, uint32_t address) {
  std::string callsign =
      fs2gdl90::protocol::SanitizeCallsign(ReadTrafficFlightId(slot));
  if (callsign.empty() && slot >= 1 &&
      slot <= g_state.traffic_text_refs.size()) {
    callsign = fs2gdl90::protocol::SanitizeCallsign(ReadDataRefText(
...
gdl90::PositionData GetOwnshipData(const Settings &cfg) {
  ...
  const bool gps_valid =
      fs2gdl90::protocol::HasValidOwnshipPosition(latitude, longitude);
  ...
  const std::string tail_number =
      fs2gdl90::protocol::SanitizeCallsign(ReadTailNumber());
  const std::string fallback_callsign =
      fs2gdl90::protocol::SanitizeCallsign(cfg.callsign);
...
bool SaveSettingsToDisk(std::string *out_error) {
  return fs2gdl90::SaveSettingsToJsonFile(g_state.settings_path,
                                          g_state.settings, out_error);
}

bool LoadSettingsFromDisk(Settings *out_settings, std::string *out_error) {
  return fs2gdl90::LoadSettingsFromJsonFile(g_state.settings_path, out_settings,
                                            out_error);
}
...
void SyncSettingsUiFromConfig() {
  fs2gdl90::SyncSettingsUiFromConfig(&g_state.settings_ui, g_state.settings);
...
bool BuildConfigFromSettingsUi(Settings *out_cfg, std::string *out_error) {
  return fs2gdl90::BuildConfigFromSettingsUi(
...
void DrawSettingsWindowUI() {
  ...
  if (ImGui::BeginTabBar("FS2GDL90Tabs")) {
  ...
  if (ImGui::Button("Defaults")) {
    fs2gdl90::LoadDefaultSettingsUiState(&g_state.settings_ui);
...
void SettingsDrawCallback(XPLMWindowID in_window_id, void *in_refcon) {
  ...
  ImGui::Begin("FS2GDL90 Settings Host", nullptr, flags);
...
void CreateSettingsWindow() {
  ...
  XPLMSetWindowTitle(g_state.settings_window, "FS2GDL90 Settings");
}
...
PLUGIN_API int XPluginStart(char *outName, char *outSig, char *outDesc) {
  std::strcpy(outName, "FS2GDL90");
  std::strcpy(outSig, "com.fs2gdl90.plugin");
  ...
  g_state.settings_path =
      std::string(prefs_path) + XPLMGetDirectorySeparator() + "fs2gdl90.json";
  ...
  const int menu_container =
      XPLMAppendMenuItem(XPLMFindPluginsMenu(), "FS2GDL90", nullptr, 0);
  g_state.menu_id =
      XPLMCreateMenu("FS2GDL90", XPLMFindPluginsMenu(), menu_container,
...
float FlightLoopCallback(float in_elapsed_since_last_call,
  ...
  if (cfg.heartbeat_rate > 0.0f &&
    ...
    const bool gps_valid = fs2gdl90::protocol::HasValidOwnshipPosition(
