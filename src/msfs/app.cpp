...
struct MsfsState {
  fs2gdl90::Settings settings;
  fs2gdl90::SettingsUiState ui_state;
...
std::string SafeString(const char *value, size_t size) {
  ...
  return fs2gdl90::msfs::TrimString(std::string(value, len));
}
...
fs2gdl90::msfs::OwnshipData ToOwnshipData(const OwnshipSimData &sim) {
  fs2gdl90::msfs::OwnshipData out;
...
fs2gdl90::msfs::TrafficData ToTrafficData(DWORD object_id,
                                          const TrafficSimData &sim) {
  fs2gdl90::msfs::TrafficData out;
...
bool ConnectSimConnect(MsfsState *state) {
  if (state->simconnect)
    return true;
  const HRESULT r = SimConnect_Open(&state->simconnect, "FS2GDL90 MSFS",
                                    nullptr, 0, nullptr, 0);
  if (FAILED(r)) {
...
void DisconnectSimConnect(MsfsState *state) {
  if (!state->simconnect)
    return;
  SimConnect_Close(state->simconnect);
  state->simconnect = nullptr;
...
void DispatchSimConnectMessage(MsfsState *state, SIMCONNECT_RECV *msg, DWORD) {
  switch (msg->dwID) {
...
void PollSimConnect(MsfsState *state) {
...
void PollForeFlightDiscovery(MsfsState *state, double now) {
  ...
  while (true) {
    ...
    uint16_t port = 0;
    if (!fs2gdl90::foreflight::ParseDiscoveryBroadcast(packet, &port))
      continue;
...
void RefreshBroadcastTarget(MsfsState *state, double now) {
  const fs2gdl90::Settings &cfg = state->settings;
...
void SendPacket(MsfsState *state, const std::vector<uint8_t> &packet) {
...
void RequestTrafficIfDue(MsfsState *state, double now) {
...
void SendScheduledPackets(MsfsState *state, double now) {
  const fs2gdl90::Settings &cfg = state->settings;
  const bool gps_valid =
      state->ownship_valid &&
      fs2gdl90::protocol::HasValidOwnshipPosition(state->ownship.latitude_deg,
  ...
  const fs2gdl90::msfs::OwnshipData own = ToOwnshipData(state->ownship);
  ...
  if (cfg.position_rate > 0.0f &&
    ...
    SendPacket(state, state->encoder->createOwnshipReport(
                          fs2gdl90::msfs::BuildOwnshipPosition(own, cfg)));
  ...
  if (now - state->last_geo_altitude >= 1.0 / kGeoAltitudeRate) {
    SendPacket(state, state->encoder->createOwnshipGeometricAltitude(
                          fs2gdl90::msfs::BuildGeoAltitude(own)));
  ...
  if (now - state->last_device_info >= 1.0 / kForeFlightDeviceRate) {
    SendPacket(state, state->foreflight_encoder->createIdMessage(
                          fs2gdl90::msfs::BuildDeviceInfo(cfg)));
  ...
  if (now - state->last_ahrs >= 1.0 / kForeFlightAhrsRate) {
    SendPacket(state, state->foreflight_encoder->createAhrsMessage(
                          fs2gdl90::msfs::BuildAhrs(own, cfg)));
    state->last_ahrs = now;
  }
  ...
  if (now - state->last_traffic >= 1.0 / kTrafficReportRate) {
    ...
    for (const TrafficEntry &entry : state->traffic) {
      ...
      if (fs2gdl90::msfs::BuildTrafficPosition(
...
bool InitializeNetworking(MsfsState *state) {
...
void ApplySettings(MsfsState *state) {
  fs2gdl90::Settings new_cfg;
  std::string error;
  if (!fs2gdl90::BuildConfigFromSettingsUi(state->ui_state, state->settings,
                                           &new_cfg, &error)) {
    state->settings_last_error = error;
    return;
  }
  if (!fs2gdl90::protocol::IsValidIpv4Address(new_cfg.target_ip)) {
  ...
  if (!fs2gdl90::SaveSettingsToJsonFile(state->settings_path, state->settings,
                                        &save_error)) {
...
void RenderUi(MsfsState *state, double now) {
  ...
  if (connected) {
    ...
    ImGui::TextColored(ImVec4(0.9f, 0.4f, 0.4f, 1.0f), "o Waiting for MSFS...");
  }
  ...
  if (ImGui::Button("Apply & Save")) {
    ApplySettings(state);
    fs2gdl90::SyncSettingsUiFromConfig(&state->ui_state, state->settings);
  }
  ImGui::EndDisabled();
  ImGui::SameLine();
  ImGui::BeginDisabled(!state->settings_dirty);
...
std::string DefaultSettingsPath() {
  ...
  if (_dupenv_s(&appdata, &sz, "APPDATA") != 0 || !appdata)
    return "fs2gdl90-msfs.json";
  std::filesystem::path p(appdata);
  free(appdata);
  return (p / "fs2gdl90" / "fs2gdl90-msfs.json").string();
}
...
void PrintUsage() {
  MessageBoxA(nullptr,
              "fs2gdl90-msfs.exe [--config PATH] [--target-ip IPv4] "
              "[--target-port PORT] [--debug]\n\n"
              "FS2GDL90 frontend for Microsoft Flight Simulator 2020/2024.\n\n"
              ...
              "FS2GDL90 MSFS", MB_OK | MB_ICONINFORMATION);
}

bool ParseArgs(int argc, char **argv, MsfsState *state) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--target-port" && i + 1 < argc) {
      ...
      if (port <= 0 || port > 65535) {
        MessageBoxA(nullptr, "Invalid target port.", "FS2GDL90 MSFS",
                    MB_OK | MB_ICONERROR);
        return false;
      }
    ...
    MessageBoxA(nullptr, ("Unknown argument: " + arg).c_str(), "FS2GDL90 MSFS",
                MB_OK | MB_ICONERROR);
    return false;
  }
  return true;
...
int main(int argc, char **argv) {
  ...
  fs2gdl90::Settings loaded = state.settings;
  if (fs2gdl90::LoadSettingsFromJsonFile(state.settings_path, &loaded,
                                         &load_error)) {
  ...
  if (!fs2gdl90::protocol::IsValidIpv4Address(state.settings.target_ip)) {
    MessageBoxA(nullptr,
                ("Invalid target IP: " + state.settings.target_ip).c_str(),
                "FS2GDL90 MSFS", MB_OK | MB_ICONERROR);
    return 1;
  }
  ...
  if (!InitializeNetworking(&state)) {
    MessageBoxA(nullptr, "Failed to initialize networking.", "FS2GDL90 MSFS",
                MB_OK | MB_ICONERROR);
    return 1;
  }
  ...
  wc.lpszClassName = L"FS2GDL90MSFS";
  wc.hIcon = LoadIcon(nullptr, IDI_APPLICATION);
  RegisterClassExW(&wc);

  g_hwnd = CreateWindowExW(
      0, L"FS2GDL90MSFS", L"FS2GDL90 MSFS", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
  ...
  UnregisterClassW(L"FS2GDL90MSFS", wc.hInstance);
