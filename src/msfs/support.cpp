...
gdl90::PositionData BuildOwnshipPosition(const OwnshipData &sim,
  ...
  data.callsign = fs2gdl90::protocol::SanitizeCallsign(sim.callsign);
  if (data.callsign.empty()) {
    data.callsign = fs2gdl90::protocol::SanitizeCallsign(cfg.callsign);
  }
...
gdl90::foreflight::DeviceInfo BuildDeviceInfo(const fs2gdl90::Settings &cfg) {
...
bool BuildTrafficPosition(const TrafficData &traffic,
  ...
  if (!out_data || !fs2gdl90::protocol::HasValidOwnshipPosition(
  ...
  data.callsign = fs2gdl90::protocol::SanitizeCallsign(traffic.callsign);
  if (data.callsign.empty()) {
    char fallback[9] = {};
