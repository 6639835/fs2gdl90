...
TEST_CASE(
  ...
  const auto missing_payload =
      fs2gdl90::test::ExtractPayload(missing_provider_msg);
...
TEST_CASE("Ownship report encodes fields and clamps values") {
  ...
  ASSERT_EQ(EncodeLat(100.0), fs2gdl90::test::Decode24(payload, 5));
  ASSERT_EQ(EncodeLon(-200.0), fs2gdl90::test::Decode24(payload, 8));
...
TEST_CASE("Ownship geometric altitude encodes altitude and vertical metrics") {
  ...
  ASSERT_EQ(static_cast<uint16_t>(0x00C8),
            fs2gdl90::test::Decode16BE(payload, 1));
  ASSERT_EQ(static_cast<uint16_t>(0x8032),
            fs2gdl90::test::Decode16BE(payload, 3));
}
