...
TEST_CASE("Simple JSON parses composite values and supports object lookup") {
  ...
  ASSERT_TRUE(fs2gdl90::json::Parse(text, &value, &error));
  ...
  const fs2gdl90::json::Value scalar;
  ASSERT_TRUE(scalar.Find("anything") == nullptr);

  ASSERT_TRUE(fs2gdl90::json::Parse("[]", &value, &error));
...
TEST_CASE("Simple JSON parses string escapes and unicode sequences") {
  ...
  ASSERT_TRUE(fs2gdl90::json::Parse(euro_text, &value, &error));
...
TEST_CASE("Simple JSON parses signed numeric formats") {
  ...
  ASSERT_TRUE(fs2gdl90::json::Parse("+42", &value, &error));
  ASSERT_EQ(42.0, value.number_value);
  ASSERT_TRUE(fs2gdl90::json::Parse("-0.25", &value, &error));
  ASSERT_EQ(-0.25, value.number_value);
  ASSERT_TRUE(fs2gdl90::json::Parse("6.02E2", &value, &error));
...
TEST_CASE("Simple JSON escape helper covers control characters") {
  ...
  ASSERT_EQ(std::string("\\\\\\\"\\b\\f\\n\\r\\t\\u0001A"),
            fs2gdl90::json::EscapeString(input));
}

TEST_CASE("Simple JSON reports required output and trailing text") {
  std::string error;
  ASSERT_TRUE(!fs2gdl90::json::Parse("null", nullptr, &error));
  ASSERT_TRUE(error.find("Output JSON value is required") != std::string::npos);

  fs2gdl90::json::Value value;
  ASSERT_TRUE(!fs2gdl90::json::Parse("null x", &value, &error));
...
TEST_CASE("Simple JSON reports string parsing failures") {
  ...
  ASSERT_TRUE(
      !fs2gdl90::json::Parse(MakeControlString('\x01'), &value, &error));
  ASSERT_TRUE(error.find("JSON strings cannot contain control characters") !=
              std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("\"unterminated", &value, &error));
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("\"\\x\"", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON string escape") != std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("\"\\u12G4\"", &value, &error));
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("\"\\uD800\\u0041\"", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON unicode escape") != std::string::npos);
}
...
TEST_CASE("Simple JSON reports number parsing failures") {
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("1e+", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON exponent") != std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("1e999", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON number") != std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("tru", &value, &error));
  ASSERT_TRUE(error.find("Unexpected end of input while parsing JSON number") !=
              std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("tx", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON number") != std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("fx", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON number") != std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("nx", &value, &error));
  ASSERT_TRUE(error.find("Invalid JSON number") != std::string::npos);
}
...
TEST_CASE("Simple JSON reports array parsing failures") {
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("[1", &value, &error));
  ASSERT_TRUE(error.find("Unexpected end of input in JSON array") !=
              std::string::npos);

  ASSERT_TRUE(!fs2gdl90::json::Parse("[1 2]", &value, &error));
...
TEST_CASE("Simple JSON reports object parsing failures") {
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("{\"a\" 1}", &value, &error));
  ...
  ASSERT_TRUE(!fs2gdl90::json::Parse("{\"a\":1 \"b\":2}", &value, &error));
