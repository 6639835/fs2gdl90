...
TEST_CASE("ForeFlight ID message encodes metadata in big-endian order") {
  ...
  info.device_name = "FS2GDL90";
  info.device_long_name = "FS2GDL90 AHRS";
  info.capabilities_mask = 0x00000005u;

  const auto message = encoder.createIdMessage(info);
  const auto payload = fs2gdl90::test::ExtractPayload(message);
...
TEST_CASE("ForeFlight AHRS message encodes attitude and heading fields") {
  ...
  ASSERT_EQ(static_cast<uint16_t>(123), fs2gdl90::test::Decode16BE(payload, 2));
  ASSERT_EQ(static_cast<uint16_t>(static_cast<int16_t>(-45)),
            fs2gdl90::test::Decode16BE(payload, 4));
  ASSERT_EQ(static_cast<uint16_t>(2712),
            fs2gdl90::test::Decode16BE(payload, 6));
  ASSERT_EQ(static_cast<uint16_t>(gdl90::foreflight::AHRS_AIRSPEED_INVALID),
            fs2gdl90::test::Decode16BE(payload, 8));
  ASSERT_EQ(static_cast<uint16_t>(gdl90::foreflight::AHRS_AIRSPEED_INVALID),
            fs2gdl90::test::Decode16BE(payload, 10));
}
...
TEST_CASE("ForeFlight AHRS message invalidates out-of-range attitude") {
  ...
  ASSERT_EQ(static_cast<uint16_t>(gdl90::foreflight::AHRS_ATTITUDE_INVALID),
            fs2gdl90::test::Decode16BE(payload, 2));
  ASSERT_EQ(static_cast<uint16_t>(gdl90::foreflight::AHRS_ATTITUDE_INVALID),
            fs2gdl90::test::Decode16BE(payload, 4));
  ASSERT_EQ(static_cast<uint16_t>(0x8000),
            fs2gdl90::test::Decode16BE(payload, 6));
  ASSERT_EQ(static_cast<uint16_t>(120), fs2gdl90::test::Decode16BE(payload, 8));
...
TEST_CASE("ForeFlight AHRS heading normalizes wraparound and invalid heading") {
  ...
  const auto wrapped_payload = fs2gdl90::test::ExtractPayload(wrapped_message);
  ASSERT_EQ(static_cast<uint16_t>(2700),
            fs2gdl90::test::Decode16BE(wrapped_payload, 6));
  ...
  const auto rounded_payload = fs2gdl90::test::ExtractPayload(rounded_message);
  ASSERT_EQ(static_cast<uint16_t>(0),
            fs2gdl90::test::Decode16BE(rounded_payload, 6));
  ...
  const auto invalid_payload = fs2gdl90::test::ExtractPayload(invalid_message);
  ASSERT_EQ(static_cast<uint16_t>(gdl90::foreflight::AHRS_HEADING_INVALID),
            fs2gdl90::test::Decode16BE(invalid_payload, 6));
}
