...
TEST_CASE("Default socket ops startup and cleanup helpers are callable") {
  ASSERT_EQ(0, udp::detail::fs2gdl90_test_default_socket_ops_startup());
  udp::detail::fs2gdl90_test_default_socket_ops_cleanup();
}

TEST_CASE("Default socket ops create socket failure path works") {
  const uintptr_t invalid =
      udp::detail::fs2gdl90_test_default_socket_ops_create_socket(-1, -1, -1);
