...
std::filesystem::path MakeTempPath(const char *suffix) {
  ...
  return std::filesystem::temp_directory_path() /
         ("fs2gdl90_" + std::to_string(now) + "_" + suffix);
}
...
TEST_CASE("Settings save and load round-trip through JSON") {
  ...
  saved.device_long_name = "Test Device";
  ...
  std::string error;
  ASSERT_TRUE(fs2gdl90::SaveSettingsToJsonFile(path.string(), saved, &error));
  ASSERT_EQ(std::string(""), error);

  fs2gdl90::Settings loaded;
  ASSERT_TRUE(
      fs2gdl90::LoadSettingsFromJsonFile(path.string(), &loaded, &error));
  ASSERT_EQ(std::string(""), error);
...
TEST_CASE("Settings save and load validate output object and file presence") {
  ...
  ASSERT_TRUE(
      !fs2gdl90::LoadSettingsFromJsonFile("missing.json", nullptr, &error));
  ...
  error = "stale";
  ASSERT_TRUE(
      !fs2gdl90::LoadSettingsFromJsonFile("missing.json", &loaded, &error));
  ASSERT_EQ(std::string(""), error);
}
...
TEST_CASE(
  ...
  ASSERT_TRUE(!loaded.foreflight_auto_discovery);
  ASSERT_EQ(static_cast<uint16_t>(63090), loaded.foreflight_broadcast_port);
  ASSERT_EQ(static_cast<uint32_t>(0x00000Fu), loaded.icao_address);
  ASSERT_EQ(std::string("LONGCALL"), loaded.callsign);
  ASSERT_EQ(std::string("DEVICE-N"), loaded.device_name);
  ASSERT_EQ(std::string("Long Device Name"), loaded.device_long_name);
  ...
  ASSERT_TRUE(!fs2gdl90::LoadSettingsFromJsonFile(scalar_path.string(), &loaded,
                                                  &error));
...
TEST_CASE("Settings saver rejects invalid target IP and write failures") {
  fs2gdl90::Settings settings;
  settings.target_ip = "not-an-ip";

  std::string error;
  ASSERT_TRUE(
      !fs2gdl90::SaveSettingsToJsonFile("/tmp/unused.json", settings, &error));
  ...
  ASSERT_TRUE(
      !fs2gdl90::SaveSettingsToJsonFile(bad_path.string(), settings, &error));
