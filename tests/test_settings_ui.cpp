...
TEST_CASE("Settings UI sync reflects config values") {
  ...
  fs2gdl90::SyncSettingsUiFromConfig(&ui_state, settings);
...
TEST_CASE("Settings UI defaults mirror default config") {
  fs2gdl90::SyncSettingsUiFromConfig(nullptr, fs2gdl90::Settings{});

  fs2gdl90::SettingsUiState ui_state;
  fs2gdl90::LoadDefaultSettingsUiState(&ui_state);
...
TEST_CASE("Settings UI builder validates fields and trims text") {
  ...
  fs2gdl90::Settings built;
  std::string error;
  ASSERT_TRUE(fs2gdl90::BuildConfigFromSettingsUi(
      ui_state, fs2gdl90::Settings{}, &built, &error));
...
TEST_CASE("Settings UI builder requires output settings object") {
  ...
  std::string error;
  ASSERT_TRUE(!fs2gdl90::BuildConfigFromSettingsUi(
      ui_state, fs2gdl90::Settings{}, nullptr, &error));
